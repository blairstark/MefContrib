...
namespace MefContrib.Integration.Unity.Ms.Tests
{
    ...
    public class ContainerExportProviderTests
    {
        ...
        public void ExportProviderResolvesServicesRegisteredByTypeTest()
        {
            ...
            Assert.AreEqual(components.Select(t => t.Value).OfType<UnityOnlyComponent1>().Count(), 1);
            Assert.AreEqual(components.Select(t => t.Value).OfType<UnityOnlyComponent2>().Count(), 1);
        }
